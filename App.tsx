...
export default function App() {
  ...
  const fetchWeather = () => {
    ...
    fetch(url)
      ...
      .catch(error => {
        ...
        Alert.alert('エラー', 'Weather data could not be loaded. Please try again.');
        setLoading(false);
      });
  };
  ...
  const getWeatherDescription = (code: number) => {
    if (code === 0) return '晴れ (Clear)';
    if (code <= 3) return '曇り (Partly Cloudy)';
    if (code <= 48) return '曇天 (Cloudy)';
    if (code <= 67) return '雨 (Rainy)';
    if (code <= 77) return '雪 (Snowy)';
  ...
  const dailyData = weather?.daily ?
    Array.from({ length: 7 }, (_, i) => {
      ...
      return {
        day: i === 0 ? '今日' : weekDays[date.getDay()],
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <LinearGradient colors={isDark ? ['#1A1A2E', '#16213E'] : [colors.primary, colors.secondary]} style={styles.header}>
        <View style={styles.headerTop}>
          <TouchableOpacity style={styles.headerContent} onPress={changeLocation}>
            ...
            <View style={styles.locationInfo}>
              <Text style={styles.locationKanji}>{TOKYO_DISTRICTS[location].kanji}</Text>
        ...
        <Text style={styles.highLow}>
          最高:{currentWeather.high}° 最低:{currentWeather.low}°
        </Text>
      </LinearGradient>
      ...
      <ScrollView style={styles.content}>
        <View style={[styles.detailsCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={styles.detailItem}>
            ...
            <Text style={[styles.detailLabel, { color: colors.textSecondary }]}>湿度</Text>
          </View>
          <View style={[styles.detailItem, styles.detailItemBorder, { borderLeftColor: colors.border, borderRightColor: colors.border }]}>
            ...
            <Text style={[styles.detailLabel, { color: colors.textSecondary }]}>風速</Text>
          </View>
          <View style={styles.detailItem}>
            ...
            <Text style={[styles.detailLabel, { color: colors.textSecondary }]}>体感温度</Text>
          </View>
        </View>

        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.text }]}>時間別予報</Text>
        ...
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.text }]}>週間予報</Text>
          {dailyData.map((day, i) => (
        ...
        <View style={styles.footer}>
          <Text style={[styles.footerText, { color: colors.textSecondary }]}>東京天気予報 • Tokyo Weather</Text>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
